...
def reporteciclico(tuplas):
	...
	if precios_originales != precios:
		if len(precios_originales)!= len(precios):
			print("--->  Se detectaron Cambios en el Inventario ")
			time.sleep(1)
			print("--->  Reiniciando...(Se Omite Reporte / se puede Notificar...)")
			time.sleep(1)
			return True
		...
		while cero<contador:
			if 0==0:
				...
				if int(precios[cero]) <10:
					concatenable="<tr> \n<th>"+ str(start)+") "+str(nombres_originales[cero])+"</th>\n"+"<th>"+str(precios_originales[cero])+"</th>\n"+"<th>"+str(precios[cero])+"</th> \n</tr>\n"
		...
		while sendMessage==False:
			try:
				...
				if response.status_code == 200:
					...
					time.sleep(2)
					if sendok>0:
						static_index=open("productos_bajos.html","w")
						void=open("templates.void").read()
